...
export default function ResultContent() {
  ...
  useEffect(() => {
    ...
    ctx.font =
      "bold 24px 'Geist', 'system-ui', 'Hiragino Kaku Gothic ProN', sans-serif";
    ...
    ctx.font =
      "900 68px 'Geist', 'system-ui', 'Hiragino Kaku Gothic ProN', sans-serif";
    ...
    <div className="pb-20 pt-8 px-4 sm:px-6 select-none">
      <div className="max-w-xl mx-auto flex flex-col items-center">
        ...
        <div className="w-full mt-10 space-y-4">
          ...
          {/* Guideline Box */}
          <div
            className={`p-4 border-2 border-slate-900 rounded-2xl transition-all duration-300 text-left w-full select-none ${copied ? "bg-emerald-50 border-emerald-500 shadow-[3px_3px_0px_rgba(52,211,153,0.35)]" : "bg-indigo-50 shadow-[3px_3px_0px_rgba(99,102,241,0.2)]"}`}
          >
            <p className="text-xs text-slate-850 font-bold leading-relaxed flex items-start gap-2">
              ...
              {copied ? (
                <span className="text-emerald-950">
                  <strong>予想画像がクリップボードにコピーされました！</strong>
                  <br />
                  開いたXの投稿画面で、
                  <strong>「貼り付け（Ctrl+V または長押し）」</strong>
                  を行うと、画像がそのまま添付されます！
                </span>
              ) : (
                <span className="text-indigo-950">
                  XでURLをポストすると、
                  <strong>予想画像が自動でカード表示されます！</strong>
                  <br />
                  また、ボタンを押すと
                  <strong>画像がクリップボードに自動コピー</strong>
                  されますので、投稿画面でそのまま
                  <strong>「貼り付け（Ctrl+V）」</strong>
                  して画像付きで投稿することも可能です！
                </span>
              )}
            </p>
...
function drawAnimal(
  ...
  else if (["EN", "NL", "BE", "MA", "SN", "ZA"].includes(code))
    animalType = "LION";
  ...
  switch (animalType) {
    ...
    default: {
      ...
      ctx.moveTo(cx - size * 0.32, cy + size * 0.16);
      ctx.lineTo(cx - size * 0.58, cy + size * 0.16);
      ctx.moveTo(cx + size * 0.32, cy + size * 0.12);
      ctx.lineTo(cx + size * 0.55, cy + size * 0.08);
      ctx.moveTo(cx + size * 0.32, cy + size * 0.16);
      ctx.lineTo(cx + size * 0.58, cy + size * 0.16);
